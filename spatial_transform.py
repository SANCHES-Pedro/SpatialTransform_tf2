...
def affine_to_shift(affine_matrix, volshape, shift_center=True, indexing='ij'):
	...
	mesh_matrix = tf.transpose(a=tf.stack(flat_mesh, axis=1))  # 4 x nb_voxels

	# compute locations
	loc_matrix = tf.matmul(affine_matrix, mesh_matrix)  # N+1 x nb_voxels
	loc_matrix = tf.transpose(a=loc_matrix[:nb_dims, :])  # nb_voxels x N
...
def meshgrid(*args, **kwargs):
	...
	**kwargs:
	...
	shapes = [tf.size(input=x) for x in args]
